...
>(({ className, ...props }, ref) => (
  <button
    ref={ref}
    className={cn(
      "absolute right-2 top-2 rounded-md p-1 text-foreground/50 opacity-0 transition-opacity hover:text-foreground focus:opacity-100 focus:outline-none focus:ring-2 group-hover:opacity-100 micro-bounce",
      className
    )}
...
// ToastProvider for rendering the toasts
const ToastProvider = ({ children }: { children: React.ReactNode }) => {
  return <div>{children}</div>
}

// ToastViewport for positioning the toasts
...
export { Toast, ToastClose, ToastTitle, ToastDescription, ToastAction, ToastProvider, ToastViewport }
